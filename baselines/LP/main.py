...
def read_graphfile(f):
    graph_data = pd.read_csv(f)
    graph_data = graph_data.values[:, [0, 1]]
    row = graph_data[:, 0]
    col = graph_data[:, 1]
    # row = np.hstack([graph_data[:,0], graph_data[:,1]])
    # col = np.hstack([graph_data[:,1], graph_data[:,0]])
    max_nid = max(np.max(row), np.max(col))
    return csr_matrix((np.ones(len(row)), (row,col)), shape=(max_nid+1,max_nid+1))
...
def read_labelfile(f):
    ...
    label_data = np.where(label_data['y'].values > 0.5, 1, 0)
    index = np.random.choice(range(label_data.shape[0]), int(label_data.shape[0] * Lambda), replace=False)
    return np.array(index, dtype=np.int32), label_data[index]
...
print('Node ID,Predicted label ID,%s' % ','.join(['Prob %s'%v for v in range(predicted.shape[1])]),\
      file=args.outfile)
for i in range(predicted.shape[0]):
    print("%s,%s,%s" % (i,predicted[i].argmax(),','.join(map(str,predicted[i]))), file=args.outfile)
