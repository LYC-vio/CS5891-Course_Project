...
def get_weight_record_features(record_file, output_dir, user_lists='all'):
    """

    :return: N(nodes) by 4 matrix of weight record features of selected users
    """
    record_frequency = dict()
    out_dict = dict()  # userID: [number of records, span, density, number of intervals]
        ...
        for line in recf:
            ...
            if line[0] != "user_id":
                ...
                try:
                    ...
                    continue
                if user_lists != 'all' and usr not in user_lists:
                    continue
                else:
                    if usr not in record_frequency:  # initialize user data
                        record_frequency[usr] = [1, [record_time,
                                                     record_time]]  # number of record, time intervals (earliest, latest)
                    else:  # update user data
                        if record_time < datetime(2008, 1,
                                                  1):  # BOOHEE was found in 2008, any records before this date should be dropped
                            continue
                        if record_time > datetime(2015, 12, 31):
                            continue
                        if (record_time - record_frequency[usr][-1][0]).days < -30:
    ...
    for key, value in record_frequency.items():
        ...
        for intv in value[1:]:
            time_span += ((intv[1] - intv[0]).days + 1)
        record_frequency[key] = [value[0], time_span, interval_num, float(value[0])/time_span]
    # print(record_frequency)
    df = pd.DataFrame.from_dict(record_frequency)
    df.to_csv(os.path.join(output_dir, 'weight_record_features.csv'), index=False)


if __name__ == '__main__':

    # weight_change_record_frequency_analysis('../data/WeightLoss/weight_record_consolidated.csv','../data/WeightLoss/clean/')
    # plot_frequency_distribution('../data/WeightLoss/clean/weight_change_record_frequency.pkl')
    get_weight_record_features('../data/WeightLoss/weight_record_consolidated.csv', '../data/WeightLoss/clean/', user_lists='all')